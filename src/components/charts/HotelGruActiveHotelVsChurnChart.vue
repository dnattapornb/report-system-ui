...
const chartData = computed(() => {
  ...
  return {
    ...
    datasets: [
      {
        label: 'Active Hotels',
        data: props.chartData.map(d => d.hotelgruHotelActual || 0),
      ...
      {
        label: 'Churned',
        data: props.chartData.map(d => d.hotelgruHotelChurnCount || 0),
...
const chartOptions: ChartOptions = {
      ...
      callbacks: {
        footer: (tooltipItems) => {
          ...
          tooltipItems.forEach(function(tooltipItem) {
            if (tooltipItem.datasetIndex === 0) activeHotels = tooltipItem.raw as number;
  ...
  scales: {
    y: {
      beginAtZero: true,
      title: { display: true, text: 'Number of Hotels' },
      ticks: {
        precision: 0
      },
      grid: {
